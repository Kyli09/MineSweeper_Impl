...
# All the directions around a cell
...
class Board:
    ...
    def __auto_open_surrounding(self,row,col):
        if self.game_board[row][col] == 0 and self.user_board[row][col] != '.':
            ...
            for dir in directions:
                if self.is_valid(row+dir[0], col+dir[1]):
                    self.__auto_open_surrounding(row+dir[0],col+dir[1])
    ...
    def is_valid_move(self,row,col):
        if row >= 0 and row < self.n and col >=0 and col < self.n and self.user_board[row][col] == "X":
            return True
        return False
        ...
        for i in range(0, self.n):
            for j in range(0, self.n):
                if self.game_board[i][j] == 'M':
                    for dir in directions:
                        if(self.is_valid(i+dir[0],j+dir[1]) and self.game_board[i+dir[0]][j+dir[1]]!='M'):
                            self.game_board[i+dir[0]][j+dir[1]] += 1
